...
describe('Emoji 컴포넌트 테스트', () => {
  ...
  it('아이콘 이름이 유효한 경우 아이콘이 렌더링된다.', () => {
    const { container } = render(<Emoji name={EMOTION.HAPPY} />);
  ...
  it('size가 없을경우, 기본값 32가 적용된다.', () => {
    ...
    const { container } = render(<Emoji name={EMOTION.HAPPY} size={defaultSize} />);
  ...
  it('size를 받을 경우, width에 값이 적용된다.', () => {
    const testSize = 40;
    const { container } = render(<Emoji name={EMOTION.HAPPY} size={testSize} />);
    const svgElement = container.querySelector('svg');
  ...
  it('className을 받을 경우, 기존 className과 함께 적용된다.', () => {
    ...
    const { container } = render(<Emoji name={EMOTION.HAPPY} className={testClassName} />);
    const svgElement = container.querySelector('svg');
  ...
  it('disabled를 true로 받을 경우, grayscale 클래스가 적용된다.', () => {
    ...
    const { container } = render(<Emoji name={EMOTION.HAPPY} disabled={true} />);
    const svgElement = container.querySelector('svg');
